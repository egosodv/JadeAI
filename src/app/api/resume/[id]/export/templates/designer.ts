...
function buildDesignerSectionContent(section: Section): string {
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div class="rounded-lg bg-zinc-50 p-3">
      <span class="text-sm font-medium text-black">${esc(it.name || it.title || it.language)}</span>
